...
```
You can see an example of an Annotation File at: https://github.com/ariadnacolmeneroc/Final-Master-Thesis-Ariadna-Colmenero/edit/Scripts/LYMPHGEN/. 
...
For HGBCL, NOS information was available for the NC. Therefore, the following code refers to the creation of the necessary files for the predictor concerning the NC.

## 4.1. CNA GENE LIST
First we create a gene list, as we have done before.
```{r}
...
To make the flat file, we need an input file containing the following columns referring to the CN event: Chromosome, Start position, End position, Length, Event. To do so:
