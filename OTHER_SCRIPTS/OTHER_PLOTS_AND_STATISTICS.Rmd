...
DRIVER_MUTATIONS_HG <- read.csv2("GENES_HG_PREDICTED_DLBCL.csv", header = F, sep = ",", stringsAsFactors = F)

colnames(DRIVER_MUTATIONS_HG) <- "GENE_SYMBOL"

DRIVER_MUTATIONS_HG <- bitr(DRIVER_MUTATIONS_HG$GENE_SYMBOL, fromType="SYMBOL", toType = "ENSEMBL", OrgDb = "org.Hs.eg.db")

DRIVER_MUTATIONS_HG <- DRIVER_MUTATIONS_HG$ENSEMBL

DRIVER_MUTATIONS_HG <- unlist(mget(DRIVER_MUTATIONS_HG, envir=org.Hs.egENSEMBL2EG,
                       ifnotfound = NA))

#Let's get the gene Universe list to do the enrichment:
...
library(org.Hs.eg.db)
data <- read.table(file = "GENE_UNIVERSE.txt", header = FALSE) # Based on the panel used of 96 genes
...
params <- new("GOHyperGParams", geneIds=DRIVER_MUTATIONS_HG ,
...
#Another example would be with KEGGHyperGParams:
library(org.Hs.eg.db)
library(clusterProfiler)
library(GOstats)
library(KEGGREST)
...
ans.kegg <- enrichKEGG(gene = DRIVER_MUTATIONS_WES,
                       organism = 'hsa')
tab.kegg <- as.data.frame(ans.kegg)
tab.kegg<- subset(tab.kegg, Count>5)
kegg <- tab.kegg[1:25, 1:6]
kegg
write.csv2(kegg, "kegg_HG.csv")

library(kableExtra)
hgOver.kegg_table <- kbl(kegg, format="html", caption="Table 4. Significative term obtained through the KEGG pathway analysis.")
kable_styling(hgOver.kegg_table, bootstrap_options = "striped", full_width = F, position = "center")
